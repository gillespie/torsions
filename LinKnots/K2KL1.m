...
a vertex, the seccond is a the passing generator, and the  
third is the outgoing generator (IPO=Incomming-Passing-Outgoing).  
    ...
    If[un0==True,
      ...
      While[MemberQ[R,{{},{}}]==False&&
          MemberQ[R,{{0},{}}]==False,  
        	...
        	NoGen++]];
         (* If[Length[pD[[1]]]==1,
                 str=str<>"Unknotting number: "<>ToString[NoGen],
                  str=str<>"Unlinking number: "<>ToString[NoGen]];
           If[Not[NoGen==1||
            SameQ[NoGen,IntegerPart[(fSignat[pD]+1)/2]]],
        str=str<>"?"]
      ];
    str *)
      NoGen=If[fSignat[pD]!=0,Range[IntegerPart[(fSignat[pD]+1)/2],NoGen],
  Range[1,NoGen]];
 NoGen=If[NoGen[[1]]==1,1,NoGen]];
  NoGen    
    ](*31.8.2004 *)
      ...
      If[StringTake[Con,1]==".",(*prva tackica*)Con=StringDrop[Con,1];
        ...
        If[StringLength[pom]>2,
          ...
          pom=StringDrop[pom,-2];
            ...
            fProjKL[pom]];(*sve projekcije datog parceta*)Do[
...
fFormDow[KKK_List,Sig_List]:=Module[{LL=KKK,q, i},
...
fColTest[Ul_,cNo_Integer]:=
  Module[{g,s,stNo,ct,i,resP={},resS={},resK={},stop=True,maxCNo,ind},
    ...
    If[resP!={},resP=Take[resP,{3,Length[resP],3}];
      Do[resP=ReplacePart[resP,{i,resP[[i]]},i],{i,1,Length[resP]}]];
    resP
    ]
(*##################################################*)
...
fDerive[b_Integer, pp_String] := 
  Module[{vv, ll, ll1, t, n1, stel, ss, ff, i, ff1, ppp, n, i, j},
    n = fAdjustNo[b, pp];
    vv = fRtan[n];
    ll = vv[[2]];
    ll1 = 
...
fGaussExtSignsBraid[Ulaz_] := Module[{Ul = Ulaz, n, pp, pp1, pp2, ppr, ppi, \
\
\
\
\
...
DFS[Ul_List] := Module[ {e = {}, dd, f},
    dd = FromUnorderedPairs[Ul]; 
    e = Sort[Map[Sort, DepthFirstTraversal[dd, 1, Edge]]];
    f = Complement[Ul, e];
    {e, f}
    ] 

PathFinder[Ul_List] := Module[{pp, i},
    pp = Table[
        ShortestPath[FromUnorderedPairs[Ul[[1]]], Ul[[2]][[i, 1]], 
          Ul[[2]][[i, 2]]], {i, Length[Ul[[2]]]}];
    pp = Table[Join[{pp[[i, -1]]}, pp[[i]]], {i, Length[pp]}];
    pp
    ]

fGrToVect[G_List, gr_List] := Module[{gr1, i},
    gr1 = Table[If[SameQ[Position[gr, G[[i]]], {}], 0, 1], {i, Length[G]}];
    gr1
    ]

fGrFromCycNew[gr_List] := Module[{gr1, i},
    gr1 = Union[Table[{gr[[i]], gr[[i + 1]]}, {i, Length[gr] - 1}]];
    gr1 = Map[Sort, gr1];
    gr1
    ]

fAllCycles[Ul_List] := Module[{tt, dd, i, j},
    tt = DFS[Ul];
    tt = PathFinder[tt];
    dd = tt;
    tt = Map[fGrFromCycNew, tt];
    tt = Table[fGrToVect[Ul, tt[[i]]], {i, Length[tt]}];
    tt = Flatten[Table[KSubsets[tt, i], {i, 2, Length[tt]}], 1];
    tt = Table[Mod[Apply[Plus, tt[[i]]], 2], {i, Length[tt]}];
    tt = Table[
        FindCycle[
          FromUnorderedPairs[
            Complement[
              Union[Table[
                  If[tt[[j, i]] == 1, Ul[[i]], {}], {i, 
                    Length[tt[[j]]]}]], {{}}]]], {j, Length[tt]}];
    tt = Union[dd, tt];
    tt
    ]

fSelectCycles[UlL_List, LL_List] := 
  Module[{gg = LL, ll, ll1, cc, eqn, ineqn, i,j},
    ll = Map[Length, gg] - 1;
    ll1 = Map[Length, Split[ll]];
    gg = Map[fGrFromCycNew, gg];
    gg = Table[fGrToVect[UlL, gg[[i]]], {i, Length[gg]}];
    gg = iteratedTake[gg, ll1];
    ll = Union[ll];
    cc = ToExpression[
        CharacterRange[FromCharacterCode[97], 
          FromCharacterCode[96 + Length[ll1]]]];
    eqn = Apply[Plus, ll*cc] == 2Length[UlL];
    ineqn = Apply[And, Table[ll1[[i]] >= cc[[i]] >= 0, {i, Length[cc]}]];
    cc = Reduce[{eqn, ineqn}, cc, Integers];
    cc = Table[
        Map[Last, 
          Sort[Table[{cc[[j, i, 1]], cc[[j, i, 2]]}, {i, 
                Length[cc[[j]]]}]]], {j, Length[cc]}];
    {gg, cc}
    ]

fSelParts[LL_List] := Module[{res, i, j},
    res = 
      Flatten[Table[
...
fSelAll[LL_List, k_Integer] := Module[{pp, j},
    j = 1; pp = {Flatten[LL[[k, 1]], 1]}; 
    While[Not[SameQ[Union[Apply[Plus, pp]], {2}]] && j <= Length[LL[[k]]], 
      pp = Flatten[LL[[k, j]], 1]; j = j + 1];
    pp = If[SameQ[Union[Apply[Plus, pp]], {2}], pp, {}];
    pp
    ]

fEdmonds[UlL_List] := Module[{gg, edg, ver, i, j, k},
    gg = fAllCycles[UlL];
    gg = If[Length[gg] == 1, {{Rest[gg[[1]]], Rest[gg[[1]]]}}, 

...
BeginPackage["LinKnots`", {"Combinatorica`",
"ComputationalGeometry`","PolyBase`",
"KnotLinkBase`","NalgBase`","KnotsByComputer`"}];

(* KnotsByComputer` is loaded above *)
(* Get["KnotsByComputer.m"]; *)
 Get["RecBase.txt"];
 Get["Classic.txt"];
(* USAGE MESSAGES *)
...
fConvert[ sInput_String, opts___Rule ] :=
  Module[ {sOut,sCh,iI,iJ,iLevel,iPos,iPrev,iNext} ,
    ...
    For[ iPos=iI=1, iI<=Length[sCh], iI++,  
      If[ lComm[[iI]] === 4 ,              
        sCh[[iI]] = "LinKnots`fBlock[" <> sCh[[iI]] <> "," <> ToString[iPos] <> "]"; 
        iPos++;                                  
        lComm[[iI]] = 1 ;       
      ];
    ];
    ...
    For[ iI=2, iI<Length[sCh], iI++, 
      If[ lComm[[iI]]===6,
        ...
        sCh   =  Insert[ sCh  , "LinKnots`fSpace[{", iPrev ];
    ...
    For[ iI=2, iI<Length[sCh], iI++,  
      If[ lComm[[iI]]===7,
        ...
        sCh   =  Insert[ sCh  , "LinKnots`fComma[{", iPrev ];
        lComm =  Insert[ lComm, 2         , iPrev ]; 
         iNext = iI+1;  
    ...
    For[ iI=2, iI<=Length[sCh], iI++,
      If[ lComm[[iI]]===5,
        ...
        sCh   =  Insert[ sCh  , "LinKnots`fPlus[{", iPrev ];
        lComm =  Insert[ lComm, 2       , iPrev ];
        iNext = iI+1;   
...
fFindCon[Ul_]:=Module[{DD,i=1,n},
    ...
    (*sada nam je DD graph spreman za izomorfizam- 
        gde da ga trazimo*)
    n=ToExpression["LinKnots`GraphA"<>ToString[n]];

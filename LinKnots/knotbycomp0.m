...
GetPdatabyTracking::usage =  
"GetPdatabyTracking[], GetPdatabyTracking[PdataFileName] 
opens a window for input a knot (or link) 
and returns its P-data using an external 
program MouseTracking.exe. In case you specified the 
PL-data file name, the knot (ot link) digagram for the
given PL-data will be appeared by right-bottom click 
in the window. In case with no data file, users can 
draw a knot (or link) on the window by 
mouse-tracking.(This mouse-tracking routine is 
based on that of SnaPea by J.Weeks.) You can change 
up-down of the crossings by left-botton click around 
the crossing, and also undo the tracking by Delete-key. 
P-data will be returned as an output when you close the 
window. This function generates a file MOUSETRACK.log, 
which has P-data (for file type) and PL-data of the 
input knot (or link) diagram, and PL-data file LastPLdata.pl."
		

ShowKnotfromPdata::usage = 
"ShowKnotfromPdata[Pdata, s:7, (options...) ] 
calls an external program DrawKnot.exe which 
calculates the coordinates of points on the 
knot (or link) required for drawing. It plots 
the image of the knot (or link) for the given 
P-data. The second parameter 's' indicates the 
number of the side-faces. Options are taken to a 
graphix command of Mathematica, Show[], directly." 
 

ShowKnotFromFile::usage ="ShowKnotFromFile[PdataFileName] 
plots the image of knots (or links) for the 
P-data (for file type) given by a file. More than one 
P-data can be given through the file, and notice that 
EOF (or 555) needs to be in the last line of the file."



GetBraidRep::usage = "GetBraidRep[Pdata] calls the 
external program BraidFromSc2.exe and the external 
program Braids-9.0 by A. Bartholomew. It returns a B-word of 
...
ShowBraid::usage ="ShowBraid[Bword,n:4,s:5] draw 
the braid obtained from B-word. B-word can be 
obtained by GetBraidRep, or you can specify by 
yourself. n and s are options; the larger 
parameters n and s are given, the smoother 
...
MutationOfTangle::usage="MutationOfTangle[P-data] 
calls an external program Mutatetangle.exe that 
operates mutate related operation on a new window. 
Tangle operation includes, flip, mutate, H-reverse, 
and V-reverse. After each operation, you can send 
the P-data to the Mathematica."




SkeinPolynomial::usage="SkeinPolynomial[type, Pdata] gives 4 kinds of
polynomials using an external program JonesPol.exe 
for the given P-data. Type 0, 1, -1, and -2 
correspond to Jones,two variable Jones, Alexander by 
Conway relation, and Conway polynomial respectively."
		

KauffmanPolynomial::usage="KauffmanPolynomial[Pdata] returns 
Kauffman polynomial for the given P-data 
using an external program KauffmanPol.exe"
		

AlexanderPolynomialbyBurauRep::usage="AlexanderPolynomialbyBurauRep[Bword] 
returns the Alexander polynomial by Burau 
representation for the given B-word 
using an external program BurauAlexPol.exe. 
Notice; Maximum length of B-word is 100, 
and maximum number of strings is 12, 
therefore characters only A(a) to L(l) are 
available."
		

ThreeParallelPolynomialInvariant::usage= \
"ThreeParallelPolynomialInvariant[Bword] 
returns one variable reduced invariant of 
the two variable Jones (or HOMFLY) polynomial 
of 3-parallel version for the given B-word 
using an external program ParallelLink.exe. 
This program needs to use a matrix file in 
the directory Braid_9, Braid_12, or Braid_15. 
Therefore be sure that these directory are 
placed in the directory K2K. 
Notice; This function needs memory so much. 
In case of lacking the memory, 
this returns nothing. Both maximum length 
of B-word and maximum number of strings depend on 
how much momory the machine, you're using, has."
		

JonesPolynomialbyBraid::usage="JonesPolynomialbyBraid[Bword] returns 
the Jones polynomial by Hecke representation 
for the given B-word using an external program 
BraidRep.exe. Notice; Maximum length of B-word 
is 100, and maximum number of strings is 12, 
therefore characters only A(a) to L(l) are 
available."
...
JonesPolynomialbyBraidFromFile::usage="JonesPolynomialbyBraid[BwordFileName] 
returns the Jones polynomials by Hecke 
representation for B-words in the B-word 
File. More than one B-word can be given 
through the file, and notice that EOF 
(or 555) needs to be in the last line of 
the file. Notice; Maximum length of B-word 
is 100, and maximum number of strings is 12, 
therefore characters only A(a) to L(l) are 
available."
...
ShowKnotbyOpengl::usage="ShowKnotbyOpengl[Pdata] 
opens a new window to draw the knot (or link) 
diagram for the given P-data rendered by OpenGL. 
You can change outer domain by left-button click 
around the center of the face to be set outer domain. 
This function calls an external program KnotOgl.exe, and 
this program calls OpenGLKnot.exe which needs OpenGLDraw.dll. 
Be sure that both OpenGLKnot.exe and OpenGLDraw.dll 
are in the directory Knot2000."




KnotFromBraid::usage="KnotFromBraid[Bword] 
returns the P-data for the given 
B-word using an external program 
BwordToPword.exe."




ReductionKnotLink::usage = "ReductionKnotLink[Pdata] 
returns a P-data of the knot (or link) which was 
reduced the total number of crossings for the given 
knot (or link) diagram using an external program 
KnotLinkReduction.exe."




SkeinPolynomialFromFile::usage= "SkeinPolynomialFromFile[type,PdataFileName] 
prints the polynomials of the type specified by 
type parameter, for knots (or links) given by 
P-data (for file type) in the file. As for type 
value, see the usage of SkeinPolynomial. 
More than one P-data can be given through the 
file, and notice that EOF (or 555) needs 
to be in the last line of the file." 
...
GetBraidRep[pdata_List]:=
	Block[
		...
		link=Install["./BraidFromSc2", LinkMode->Launch]; 
		gb = GetBraid[pdata[[2]],pdata[[1]]];
		gb=If[SameQ[Head[gb],List]&&SameQ[Apply[Plus,gb[[3]]],Length[gb[[2]]]]||\
SameQ[Head[gb],String],gb,{}];
			While[Length[gb] == 3&&SameQ[Apply[Plus,gb[[3]]],Length[gb[[2]]]],
			pgb = {gb[[3]],gb[[2]]};
			(* Print[pgb]; *)
			gb = GetBraid[gb[[2]],gb[[3]]];
			];
		Uninstall[link];
		gb=If[SameQ[Head[gb],List],gb={},gb];
		gb=If[Not[SameQ[gb,{}]]&&SameQ[StringPosition[gb,"."],{}],gb,\
fGetBraidRepresent[pdata]];
		gb
	]	
...
ThreeParallelPolynomialInvariant[bword_] := 
  Block[{i, x, q, bbwd, polstr, c, st, wythe, ex, jonespol},
    bbwd = bwdtonumwd[bword];
    c = Length[bbwd];
   If[c > 100, Print["B-word is too long."]; Break[]];
    st = Abs[bbwd[[1]]];
    wythe = 0;
   If[bbwd[[1]]>0, wythe++, wythe--];
    For[i=2, i<=c, i++, 
      If[bbwd[[i]]>0, wythe++,wythe--];
      If[Abs[bbwd[[i]]]>= st,st=Abs[bbwd[[i]]]]];
    bbwd = Prepend[bbwd, st+1];
    link = Install["./ParallelLink", LinkMode -> Launch];
    polstr = ParallelRep[First[bbwd], bbwd];
    wythe = 3*wythe;
    jonespol = ToExpression[polstr];
    jonespol = ReplaceAll[jonespol,x -> Sqrt[q]];
    ex = (q^(3/2))^wythe;
    jonespol = ex*jonespol;
    ex = (1-q^4)/(q^(3/2)(1-q));
    jonespol = jonespol*ex^(3*(st+1)-1);
    ex = q^2*(1+q+q^2+q^3)^(-3(st+1))/(1+q+q^2+q^3+q^4);
    jonespol = jonespol*ex;
    Print["3-parallel one variable invariant for ", "[", bword, "]"];
    Uninstall[link];
    jonespol = Simplify[Expand[jonespol]];
    If[SameQ[jonespol, -1], jonespol = 1];
    If[Not[SameQ[jonespol, 1]], jonespol = Simplify[Divide[jonespol, ex]]];
    Return[jonespol] 
    ]
...
GetBraidRepFromFile[str_String,otype_Integer:0]:=
    ...
    While[True,
      ...
      If[rep<n && MemberQ[ToCharacterCode[gb],64],
        pdata=ReductionKnotLink[pdata];Print[totalcount,":Retry-> \
",rep];rep++;
        Goto[back]];
      If[otype\[Equal]0,
        	Print[gb],
...
\
    ...
    Print[{nump,pdata}];
        (* Print[
          "Outputted to PLFromSnappea.sp which is a data file of the function \
\
\
\
\
 ...
 SnapPeaDataFromPLData[fname_]:=
  Block[{i,j,k=0,sn,ex,len,com,crossnum={},compnumxy={},xy={},x,y,x0,y0,
      ...
      If[i==compnumxy[[j]],Write[fileid,i-1," ",k];k=compnumxy[[j]]; j++, 
        ...
        Print[
      "Outputted to SnappeaFromPL.pl which must been only used in J. Weeks's \
\
\
\
\
...
KnotFromPlat[str_]:=Block[{i,j,len,bnum,bindex,plat,pd,knot,nump},
    ...
    If[Mod[bindex,2]\[Equal]1,
      Print[
        "Wrong data !!! (All braids must have even strings for plat \
closure.)"];Return[str]];
